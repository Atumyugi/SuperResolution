...
def WDSR_eval(testimg_path,labelimg_path,test_directory_path,label_directory_path,wdsr_model):
    ...
    out = crop_input_return_out(img, height, weight, wdsr_model)
...
def crop_input_return_out(img,height,weight,wdsr_model):
...
# test
def WDSR_create_srimg(testimg_path,test_directory_path,save_path,wdsr_model):
    lr_path = test_directory_path + testimg_path
    img = cv2.imread(lr_path, cv2.IMREAD_COLOR)
    height, weight, _ = img.shape
    out = crop_input_return_out(img,height,weight,wdsr_model)
...
def WDSR_super_resolution(testimg_path,labelimg_path,test_directory_path,label_directory_path,txt_path,save_path,wdsr_model):
    ...
    f.write(str(PSNR(source_img_y, img_cubic_y))+'\n')
    f.write('PSNR:WDSR_Y'+'\n')
    f.write(str(PSNR(source_img_y, out_y))+'\n')
    f.write('PSNR:CUBIC'+'\n')
    f.write(str(PSNR(source_img, img_cubic))+'\n')
    f.write('PSNR:WDSR'+'\n')
    f.write(str(PSNR(source_img, out))+'\n')
    f.write('SSIM:CUBIC'+'\n')
    f.write(str(SSIM(source_img, img_cubic))+'\n')
    f.write('SSIM:WDSR'+'\n')
    f.write(str(SSIM(source_img, out))+'\n')
    f.close()
    print(testimg_path,'finished......')
    return PSNR(source_img, img_cubic),PSNR(source_img, out)
